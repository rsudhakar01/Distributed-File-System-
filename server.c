...
int server_create(message_t m){
	...
	pinode[check_freeinode].type = dt;
	...
	if(dt == MFS_REGULAR_FILE){
		for(int i = 0; i < DIRECT_PTRS; i++) {
			...
			pinode[check_freeinode].direct[i] = new_datab_ptr + dregion_ptr;
		...
		pinode[check_freeinode].size = 2 * sizeof(dir_ent_t);
		...
		for(int i = 1; i < DIRECT_PTRS; i++) {
			pinode[check_freeinode].direct[i] = -1;
		}
		...
		pinode[check_freeinode].direct[0] = new_datab_ptr + dregion_ptr;
		set_bit((unsigned int *)dbitmap_ptr, new_datab_ptr);
